...
TEST(WasmCov3, WriteEmptyProgram) {
    ASTNode *ast = parse_code("");
    if (ast) {
        int ret = wasm_write_program(ast, "/tmp/test_wasm_empty_cov3.wasm", TARGET_WASI_P2);
        (void)ret;
        unlink("/tmp/test_wasm_empty_cov3.wasm");
        ast_node_free(ast);
    }
}

TEST(WasmCov3, WriteNullOutput) {
