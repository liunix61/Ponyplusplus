...
TEST(CodegenCov3, SourceMapSaveLoadMultiple) {
    ...
    int ret = sourcemap_save_json(sm, "/tmp/test_sourcemap_cov3.json");
    EXPECT_EQ(ret, 0);
    SourceMap *loaded = sourcemap_load_json("/tmp/test_sourcemap_cov3.json");
    ...
    unlink("/tmp/test_sourcemap_cov3.json");
    sourcemap_free(sm);
}

TEST(CodegenCov3, SourceMapSaveLoadEmpty) {
    SourceMap *sm = sourcemap_new();
    EXPECT_NE(sm, nullptr);
    int ret = sourcemap_save_json(sm, "/tmp/test_sourcemap_empty_cov3.json");
    EXPECT_EQ(ret, 0);
    SourceMap *loaded = sourcemap_load_json("/tmp/test_sourcemap_empty_cov3.json");
    EXPECT_NE(loaded, nullptr);
    if (loaded) {
        sourcemap_free(loaded);
    }
    unlink("/tmp/test_sourcemap_empty_cov3.json");
    sourcemap_free(sm);
}

/* ==================== codegen_set_source_file 更多情况 ==================== */

TEST(CodegenCov3, SetSourceFileEmpty) {
    FILE *out = fopen("/dev/null", "w");
    if (!out) return;
    Codegen *cg = codegen_new(out);
    if (cg) {
...
TEST(CodegenCov3, SetSourceFilePath) {
    FILE *out = fopen("/dev/null", "w");
    if (!out) return;
    Codegen *cg = codegen_new(out);
    if (cg) {
...
TEST(CodegenCov3, NewWithMapAndSourceFile) {
